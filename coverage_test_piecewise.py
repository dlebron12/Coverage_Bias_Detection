...
"""Get summary info per position 20-80 for the 8 QC Samples
Spec > 1.3 coverage may indicate bias
"""
...
if __name__=='__main__':
    ...
    for c in qc.columns:
        ...
        if (round(max(lower_coverage),2) > 1.4 or round(max(upper_coverage),2) > 1.4):
            ### max coverage or average of the coverage at 0-10 or 80-100 ?? I think average is better :/ 
            print(f"Sample: {samplename} failed with a slope of:{m} -5 or 3' bias'")
            passed= False
            
        elif (round(max(lower_coverage),2) >= 1.2 and round(max(lower_coverage),2) <= 1.4) or (round(max(upper_coverage),2) >=1.2 and round(max(upper_coverage),2) <= 1.4 ):
            m2,b2=Calculate_Slope(sample,(0,10))
